...
# Removed duplicate /trends endpoint - keeping only /roi/trends
...
async def get_campaign_markers(
    ...
    range: str = Query("7d", description="Time range: 7d, 14d, 30d, 90d, or custom"),
    db = Depends(get_db),
):
    try:
        ...
        print(f"📅 Range parameter received: {range}")
...
async def get_monthly_spend_trends(
    ...
    try:
        ...
        # Use Supabase to get monthly spend trends data
        print(f"🔍 Monthly Spend Trends query params: {query_params}")
        response = await supabase_client._make_request(
            "GET",
            "roi_metrics",
            params=query_params
...
async def export_csv(
    ...
    try:
        ...
        # Use Supabase to get ROI metrics data
        print(f"🔍 Export CSV query params: {query_params}")
        response = await supabase_client._make_request(
            "GET",
            "roi_metrics",
            params=query_params
...
async def export_pdf(
    ...
    try:
        ...
        # Use Supabase to get ROI metrics data
        print(f"🔍 Export PDF query params: {query_params}")
        response = await supabase_client._make_request(
            "GET",
            "roi_metrics",
            params=query_params
        ...
        page = doc.new_page()
        y = 50
        page.insert_text((50, y), f"ROI Report (All Data)", fontsize=14)
        y += 30
        for r in rows:
...
async def generate_ai_report(
    user_id: str = Query(None, description="User ID (not used for data filtering)"),
    db = Depends(get_db),
):
    """
    ...
    try:
        ...
        try:
            from app.services.roi.report_generation.roi_report_service import ROIReportService
        except ImportError as e:
            print(f"Failed to import ROIReportService: {e}")
            raise HTTPException(
                status_code=500, 
                detail="Report generation service not available. Please ensure the service modules are properly installed."
            )
        ...
        if not result.get("success", False):
            raise HTTPException(
                status_code=500,
                detail=f"Failed to generate ROI report: {result.get('message', 'Unknown error')}"
            )
...
@router.post("/generate-html-report", tags=["roi"])
async def generate_html_report(
    user_id: str = Query(None, description="User ID (not used for data filtering)"),
    db = Depends(get_db),
):
    """
    Generate an HTML-only ROI report using AI analysis.
    This endpoint generates HTML without PDF conversion for faster response times.
    """
    try:
        # Import the HTML report generation service
        from app.services.roi.report_generation.roi_report_service import ROIReportService
        
        print("Generating HTML-only ROI report...")
        
        # Generate the HTML report using the service
        service = ROIReportService()
        result = await service.generate_html_only(user_id)
        
        if not result.get("success", False):
            raise HTTPException(
                status_code=500,
                detail=f"Failed to generate HTML report: {result.get('message', 'Unknown error')}"
            )
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"generate_html_report failed: {str(e)}")
