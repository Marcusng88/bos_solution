...
CREATE TYPE social_media_platform AS ENUM ('instagram', 'facebook', 'twitter', 'linkedin', 'youtube', 'other');
...
-- ============================================================================
-- NEW TABLE: SOCIAL MEDIA INSIGHTS (for hourly/daily metrics snapshots)
-- ============================================================================

-- Stores time-bucketed insights for FB/IG accounts to support analytics
CREATE TABLE IF NOT EXISTS social_media_insights (
    ...
    metrics JSONB NOT NULL, -- Raw metrics keyed by API metric name
    derived JSONB, -- Optional computed fields
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT smi_unique UNIQUE (user_id, platform, account_id, period, window_start)
);

CREATE INDEX IF NOT EXISTS idx_smi_user_platform ON social_media_insights(user_id, platform);
CREATE INDEX IF NOT EXISTS idx_smi_account_period ON social_media_insights(account_id, period);
CREATE INDEX IF NOT EXISTS idx_smi_window_start ON social_media_insights(window_start);
