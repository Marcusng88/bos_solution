  ...
  persist(
    (set, get) => ({
      ...
      refreshToken: async () => {
        ...
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/youtube/auth/refresh`, {
            method: 'POST',
            headers: {
      ...
      getROIAnalytics: async (daysBack = 30, includeRevenue = true) => {
        ...
        try {
          let url = `${process.env.NEXT_PUBLIC_API_URL}/youtube/analytics/roi-dashboard?access_token=${get().tokens?.access_token}&days_back=${daysBack}&include_estimated_revenue=${includeRevenue}`
